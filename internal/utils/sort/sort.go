...
func GetSort(columns []string, table string, sorts []*arceus.SortMethod) (func(s *sql.Selector), error) {
	var values []string
	for _, data := range sorts {
		if Contains(columns, data.Name) {
			switch data.Type {
			case arceus.SortType_SORT_TYPE_ASC:
				values = append(values, sql.Asc(fmt.Sprintf("`%s`.`%s`", table, data.Name)))
			case arceus.SortType_SORT_TYPE_DESC:
